...
const Marketplace = () => {
  ...
  const [formInput, updateFormInput] = useState({ rentalDuration: "5" });
  ...
  async function loadNFTs() {
    ...
    const [txn, rentalArray, tokenArray] =
      await gameContract.fetchMarketItems();
    console.log(
      "MarketItem Information",
      await gameContract.fetchMarketItems()
    );
    if (txn[0]) {
      ...
      const tokenItems = await Promise.all(
        tokenArray.map(async (tokenData) => {
      ...
      const rentalItems = await Promise.all(
        rentalArray.map(async (rentalData) => {
          let item = {
  ...
  async function rentNFT(tokenID, price) {
    ...
    const finalRentalPrice = ethers.utils.parseUnits(
    ...
    const listTxn = await gameContract.rentNFT(
      tokenID,
      rentalDuration,
      overrides
    );
                ...
                <div key={i} className="grid-item">
                  ...
                  <div className="wrapper6">
                    ...
                    <Flex alignItems="center" mt="10px" mb="10px">
                      ...
                      <Text fontSize="14px" ml="5px" bold>
                        {nft.attackDamage}
                      </Text>
                    </Flex>

                    <Flex alignItems="center" mt="10px" mb="10px">
                      <Icon icon="cryptocurrency:eth" style="color:#ff4655;" />
                      <Text fontSize="14px" ml="5px" mr="30px" bold>
                        {ethers.utils.formatEther(
                          renft[i].price.toString(),
                          "ether"
                        )}
                      </Text>
                      <Icon icon="flat-color-icons:alarm-clock" />
                      <Text fontSize="14px" ml="5px" bold>
                        {renft[i].maxRental.toString()} Mins
                      </Text>
                    </Flex>

                    <TextField
                      label="Rental Duration"
                      defaultValue={renft[i].maxRental.toString()}
                      variant="filled"
                      type="number"
                    ...
                    >
                      <span className="front">Rent NFT</span>
                    </button>
                  </div>
                </div>
              ))}
